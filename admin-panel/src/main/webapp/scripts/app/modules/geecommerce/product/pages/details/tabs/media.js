define([ 'durandal/app', 'knockout', 'gc/gc', 'gc-product' ], function(app, ko, gc, productAPI) {
    ...
    ProductMediaController.prototype = {
        ...
        saveData : function(view, parent, toolbar, args) {
            ...
            var updates = [];
            var pVM = _.findWhere(self.products(), {
                id : args.productId
            });
            ...
            if (!_.isEmpty(pVM)) {
                ...
                if (!_.isEmpty(mediaAssets)) {
                    ...
                    _.each(mediaAssets, function(data) {
                        ...
                        var updateModel = gc.app.newUpdateModel().id(data.id).field('title', data.title(), true).field('mediaTypeIds', data.mediaTypeIds() || []).field('position', data.position() || 99);
        ...
        removeMediaAsset : function(mediaVM, event) {
            ...
            productAPI.removeMediaAsset(mediaVM.id, mediaVM.productId).then(function(data) {
                var pVM = _.findWhere(self.products(), {
                    id : mediaVM.productId
                });
                var mVM = _.findWhere(pVM.media(), {
                    id : mediaVM.id
                });
        ...
        addMediaAsset : function(productId, file, dz) {
            ...
            if (!_.isUndefined(productId) && !_.isUndefined(file)) {
                ...
                productAPI.uploadMediaAsset(productId, fd).then(
                    function(mediaAsset) {
                        var pVM = _.findWhere(self.products(), {
                            id : '' + productId
                        });
                        var mediaVM = new MediaVM(mediaAsset.id, mediaAsset.productId, mediaAsset.firstStoreId, mediaAsset.path, mediaAsset.webPath, mediaAsset.webThumbnailPath, mediaAsset.previewImagePath, mediaAsset.previewImageWebPath,
                            mediaAsset.previewImageWebThumbnailPath, mediaAsset.title, mediaAsset.position, mediaAsset.mimeType, mediaAsset.mediaTypeIds, mediaAsset.variantDefault, mediaAsset.bundleDefault, mediaAsset.programmeDefault,
                            mediaAsset.enabled);
        ...
        refreshMediaAssets : function(productId) {
            ...
            productAPI.getMediaAssets(productId, [ 'image/*', 'video/*' ]).then(
                function(data) {
            ...
            $(document).on(
                ...
                function() {
                    ...
                    return productAPI.getProduct(self.productId).then(function(product) {
                        ...
                        function(data) {
                            productAPI.getVariants(self.productId).then(
                                function(data) {
                                    ...
                                    _.each(products, function(product) {
                                        ...
                                        self.products.push(new ProductVM(product.id, gc.attributes.find(product.attributes, 'article_number').value, gc.attributes.find(product.attributes, 'name').value,gc.attributes
                                            .find(product.attributes, 'name2').value, variantValues));
                                    });
                                    // ---------------------------------------------------------------------------
                                ...
                                function(data) {
                                    productAPI.getMediaAssets(self.productId, [ 'image/*', 'video/*' ]).then(
                                        function(data) {
                                            _.each(data.data.catalogMediaAssets, function(mediaAsset) {
                                                var pVM = _.findWhere(self.products(), {
                                                    id : mediaAsset.productId
                                                });
                                                var mediaVM = new MediaVM(mediaAsset.id, mediaAsset.productId, mediaAsset.firstStoreId, mediaAsset.path, mediaAsset.webPath, mediaAsset.webThumbnailPath, mediaAsset.previewImagePath,
                                                    mediaAsset.previewImageWebPath, mediaAsset.previewImageWebThumbnailPath, mediaAsset.title, mediaAsset.position, mediaAsset.mimeType, mediaAsset.mediaTypeIds, mediaAsset.variantDefault,
                                                    mediaAsset.bundleDefault, mediaAsset.programmeDefault, mediaAsset.enabled);
                                                pVM.media.push(mediaVM);
                                            });
                                        });
                                    // ---------------------------------------------------------------------------
                                ...
                                function(data) {
                                    ...
                                    $('.dropzone-image-preview').livequery(function() {
                                        $form = $(this).get(0);
                                        var productId = $(this).data('productid');
                                        var mediaAssetId = $(this).data('id');

                                        var dz = new Dropzone($form, {
                                            method : 'put',
                                            url : '/api/v1/products/' + productId + '/media-assets/' + mediaAssetId + '/preview-image',
                                            thumbnailWidth : 50,
                                            thumbnailHeight : 50
                                        });

                                        dz.on("addedfile", function(file) {
                                            var ezMessageEL = $(file.previewElement).closest('form').children('.dz-message').first();
                                            ezMessageEL.hide();
                                        });

                                        dz.on("success", function(file, data) {
                                            var mediaAsset = data.data.catalogMediaAsset;
                                            var ezMessageEL = $(file.previewElement).closest('form').children('.dz-message').first();

                                            var pVM = _.findWhere(self.products(), {
                                                id : '' + productId
                                            });
                                            var mediaVM = _.findWhere(pVM.media(), {
                                                id : '' + mediaAssetId
                                            });

                                            if (!_.isEmpty(mediaVM)) {
                                                mediaVM.previewImagePath(mediaAsset.previewImagePath);
                                                mediaVM.previewImageWebPath(mediaAsset.previewImageWebPath);
                                                mediaVM.previewImageWebThumbnailPath(mediaAsset.previewImageWebThumbnailPath);
                                            }
