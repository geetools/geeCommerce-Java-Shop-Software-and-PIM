...
var momentDatePickerKo = new MomentDatePickerKo();
...
ko.bindingHandlers.datepicker = {
        init: function (element, valueAccessor, allBindingsAccessor) {
            ...
            var dataType = options.dataType || momentDatePickerKo.detectDataType(ko.utils.unwrapObservable(valueAccessor()));

            dataType = !dataType || dataType == 'string' ? 'iso' : dataType;
            var $el = $(element).datepicker(options);

            momentDatePickerKo.elBinder($el).register(dataType);

            ko.utils.registerEventHandler(element, "changeDate", function (event) {
                var accessor = valueAccessor();
                if (ko.isObservable(accessor)) {
                    var value = momentDatePickerKo.elBinder($el).get();
                    accessor(value);
                }
            });
        },
        update: function (element, valueAccessor) {
        	momentDatePickerKo.elBinder($(element))
