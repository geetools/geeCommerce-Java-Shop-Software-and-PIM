...
public class Observers {
	...
	String key = new StringBuilder(modelClass.getName()).append(OBSERVERS_CACHE_KEY_PART).append(e.name()).toString();
	...
	if (observerClasses == null) {
	    ...
	    List<Class<Observer>> cachedObserverClasses = (List<Class<Observer>>) cache.putIfAbsent(key, observerClasses);
	    ...
	    // Iterate through them and find Observer observing 'this' class for the event passed to this method
		...
		if (declaredAnnotation != null) {
		    ...
		    if ((!"".equals(name.trim()) && modelClass.getName().endsWith(name.trim()) && Arrays.asList(eventToObserve).contains(e))
			    || (allInterfaces != null && allInterfaces.size() > 0 && !"".equals(name.trim()) && interfacesMatch(allInterfaces, name.trim()) && Arrays.asList(eventToObserve).contains(e))) {
