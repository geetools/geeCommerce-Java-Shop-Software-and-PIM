...
public class ModelTransformer implements ClassFileTransformer {
    ...
    public byte[] transform(ClassLoader loader, String className, Class<?> classBeingRedefined,
        ...
        System.out.println("1----> " + classBeingRedefined + "  -  " + className + " - "
            + ("to/test/invokedynamic/TestClass".equals(className)));
        ...
        try {
            ...
            boolean isModel = modelImplClass.hasAnnotation(com.geecommerce.core.service.annotation.Model.class)
                && modelImplClass.subtypeOf(modelClass);
                ...
                if (f.hasAnnotation(Column.class)) {
                    try {
                        if (!Modifier.isFinal(f.getModifiers()) && !Modifier.isStatic(f.getModifiers())) {
                            ...
                            CtField staticField = new CtField(cp.get(STRING_CLASS_NAME),
                                STATIC_FIELD_PREFIX + f.getName(), modelImplClass);
    ...
    private String buildMethodBody(CtClass modelImplClass) {
        ...
        // body.append("java.util.List<com.geecommerce.core.service.ColumnInfo>
        // columnInfos =
        // com.geecommerce.core.service.Annotations.getColumns(modelClass);");
        ...
        if (fields != null && fields.length > 0) {
            ...
            for (CtField f : fields) {
                if (f.hasAnnotation(Column.class)) {
                    try {
                        if (!Modifier.isFinal(f.getModifiers()) && !Modifier.isStatic(f.getModifiers())) {
                            // body.append(" case ").append(STATIC_FIELD_PREFIX
                            // + f.getName()).append(":\n");
                            // body.append("
                            // ").append(buildSetter(f)).append("((").append(f.getType().getName()).append(")").append("value);\n");
                            // body.append(" break;\n");
                            ...
                            body.append("        ").append(buildSetter(f)).append("((").append(f.getType().getName())
                                .append(")").append("value);\n");
                            body.append("    }\n");
        ...
        // System.out.println(\"set(String field, Object value) IN NEW METHOD!!
        // \"); }"
    ...
    private String buildSetter(CtField field) {
        ...
        StringBuilder setter = new StringBuilder("set").append(fieldName.substring(0, 1).toUpperCase())
