...
/**
 * Abstract class used for extending implementations of method interceptors. The {@link com.geecommerce.core.interceptor.AspectMethodInterceptor} takes care of
 * aspect handling and finding matching method interceptors that extend this class. The extending method interceptor must also annotate the class
 * definition with the annotation {@link com.geecommerce.core.interceptor.annotation.Intercept}, otherwise it will not be found. Note that it is not sufficient
 * to <i>only</i> annotate the method interceptor. The class to be intercepted must also allow this with the
