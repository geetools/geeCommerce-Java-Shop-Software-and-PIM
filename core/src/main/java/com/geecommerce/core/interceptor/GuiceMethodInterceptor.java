...
public class GuiceMethodInterceptor implements MethodInterceptor {
	...
	String key = new StringBuilder(invocationClass.getName()).append(Char.DOT).append(invocation.getMethod().toString()).toString();
	...
	if (interceptorTypes == null) {
	    ...
	    // Iterate through all the @Intercept classes and find one (or more) that matches the target joinPoint-type
	    // and
		...
		if (declaredAnnotation != null) {
		    ...
		    // All interfaces from the invocation class (also from sub-types).
		    Set<Class<?>> allInterfaces = Reflect.getInterfaces(invocationClass, true);

		    if ((!Object.class.equals(classToIntercept) && classToIntercept.equals(invocationClass) && methodToIntercept.equals(invocationMethod))
			    || (Object.class.equals(classToIntercept) && !"".equals(name.trim()) && invocationClass.getName().endsWith(name.trim()) && methodToIntercept.equals(invocationMethod))
			    || (allInterfaces != null && allInterfaces.size() > 0 && Object.class.equals(classToIntercept) && !"".equals(name.trim()) && interfacesMatch(allInterfaces, name.trim()) && methodToIntercept.equals(invocationMethod))) {
			interceptorTypes.add(new LocatedMethodInterceptor((Intercept) declaredAnnotation, (Class<AbstractMethodInterceptor>) annotatedType));
		    }
		}
	    }

	    if (interceptorTypes != null && interceptorTypes.size() > 0) {
		// Sort method interceptors according to the specified order in Intercept annotation.
		Collections.sort(interceptorTypes, new Comparator<LocatedMethodInterceptor>() {
			...
			return (o1.interceptorAnnotation.order() < o2.interceptorAnnotation.order() ? -1 : (o1.interceptorAnnotation.order() > o2.interceptorAnnotation.order() ? 1 : 0));
		    }
		});
	    }

	    List<LocatedMethodInterceptor> cachedInterceptorTypes = (List<LocatedMethodInterceptor>) cache.putIfAbsent(key, interceptorTypes == null ? EMPTY_LIST : interceptorTypes);
