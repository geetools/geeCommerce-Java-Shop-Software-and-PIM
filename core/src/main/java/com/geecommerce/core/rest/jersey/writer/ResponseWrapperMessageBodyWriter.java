...
public class ResponseWrapperMessageBodyWriter implements MessageBodyWriter<ResponseWrapper> {
    ...
    @Override
    public long getSize(ResponseWrapper t, Class<?> type, Type genericType, Annotation[] annotations, MediaType mediaType) {
	return -1;
    }

    @SuppressWarnings("unchecked")
    @Override
    public void writeTo(ResponseWrapper responseWrapper, Class<?> type, Type genericType, Annotation[] annotations, MediaType mediaType, MultivaluedMap<String, Object> httpHeaders, OutputStream entityStream) throws IOException, WebApplicationException {
	...
	for (String key : keys) {
		...
		if (value instanceof List) {
			...
			if (Model.class.isAssignableFrom(o.getClass())) {
			    JAXBWriterHelper.write((List<Model>) list, type, o.getClass().getAnnotations(), mediaType, httpHeaders, entityStream);
			}
		    }
		} else {
		    JAXBContext jaxbContext = JAXBContextFactory.INSTANCE.getJaxBContext(value.getClass());
