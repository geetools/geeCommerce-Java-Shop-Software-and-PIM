...
public class MultiContextCacheKey {
    ...
    public <T extends Model> MultiContextCacheKey(Class<T> modelClass, Map<String, Object> filter, String distinctFieldName, QueryOptions queryOptions) {
	StringBuilder cacheKey = new StringBuilder(CACHE_KEY_QUERY_PREFIX).append(modelClass.getName()).append(Char.SLASH);
    ...
    public <T extends Model> MultiContextCacheKey(Class<T> modelClass, RequestContext reqCtx, Map<String, Object> filter, String distinctFieldName, QueryOptions queryOptions) {
	StringBuilder cacheKey = new StringBuilder(CACHE_KEY_QUERY_PREFIX).append(modelClass.getName()).append(Char.SLASH);

	cacheKey.append(REQUEST_CONTEXT_PREFIX).append(reqCtx == null || reqCtx.getId() == null ? 0 : reqCtx.getId().longValue()).append(Char.SLASH);
    ...
    public <T extends Model> MultiContextCacheKey(Class<T> modelClass, Store store, Map<String, Object> filter, String distinctFieldName, QueryOptions queryOptions) {
	StringBuilder cacheKey = new StringBuilder(CACHE_KEY_QUERY_PREFIX).append(modelClass.getName()).append(Char.SLASH);
    ...
    public <T extends Model> MultiContextCacheKey(Class<T> modelClass, Merchant merchant, Map<String, Object> filter, String distinctFieldName, QueryOptions queryOptions) {
	StringBuilder cacheKey = new StringBuilder(CACHE_KEY_QUERY_PREFIX).append(modelClass.getName()).append(Char.SLASH);
