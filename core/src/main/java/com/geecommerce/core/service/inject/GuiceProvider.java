...
public class GuiceProvider<T> implements Provider<T> {
    ...
    public GuiceProvider(Class<T> interfaceClass, Class<T> defaultClass) {
        if (log.isTraceEnabled()) {
            log.trace("Registering GuiceProvider for [interfaceClass=" + interfaceClass + ", defaultClass="
                + defaultClass + ", interfaceClassLoader=" + interfaceClass.getClassLoader()
                + ", defaultClassLoader=" + defaultClass.getClassLoader() + "].");
        }
    ...
    public T get() {
        try {
            ...
            if (loadedClass == null) {
                ...
                if (log.isTraceEnabled()) {
                    log.trace("Attempting to load class from GuiceProvider [interfaceClass=" + interfaceClass
                        + ", defaultClass=" + defaultClass + ", interfaceClassLoader="
                        + interfaceClass.getClassLoader() + ", defaultClassLoader=" + defaultClass.getClassLoader()
                        + "].");
                }
            ...
            if (log.isTraceEnabled()) {
                log.trace(
                    "Loaded class '" + loadedClass + "' with classloader '" + loadedClass.getClassLoader() + "'.");
            }

            T obj = (T) injector.getInstance(loadedClass);

            if (log.isTraceEnabled()) {
                log.trace("Returning object created by Guice: " + obj + ", classloader="
                    + (obj != null ? obj.getClass().getClassLoader() : null));
            }
