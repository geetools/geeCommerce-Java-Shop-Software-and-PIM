...
public class ConfigurationResource extends AbstractResource {
	...
	List<ConfigurationProperty> cp = service.get(ConfigurationProperty.class, filter.getParams(), queryOptions(filter));
	return ok(checked(cp));
	// return ok(checked(service.get(ConfigurationProperty.class, filter.getParams(), queryOptions(filter))));
    }
