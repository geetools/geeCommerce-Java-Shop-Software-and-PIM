...
public class DefaultRole extends AbstractModel implements Role {
    ...
    public List<Permission> getPermissions() {
        if (permissionsList == null && permissionIds != null && permissionIds.size() > 0) {
            permissionsList = permissions.findByIds(Permission.class,
                permissionIds.toArray(new Id[permissionIds.size()]));
        }
    ...
    public String toString() {
        return "DefaultRole [id=" + id + ", code=" + code + ", name=" + name + ", permissionIds=" + permissionIds
