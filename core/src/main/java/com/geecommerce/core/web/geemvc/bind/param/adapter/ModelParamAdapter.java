...
public class ModelParamAdapter implements TypedParamAdapter<ModelParam> {
    ...
    @Inject
    protected ModelParamAdapter(ParamAdapters paramAdapters, DataAdapterFactory dataAdapterFactory,
        ReflectionProvider reflectionProvider, SimpleConverter simpleConverter,
        BeanConverterAdapterFactory beanConverterAdapterFactory) {
    ...
    public Object getTypedValue(ModelParam modelParam, String paramName, ParamContext paramCtx) {
        ...
        Class<?> beanClass = reflectionProvider.getPrimaryType(paramCtx.methodParam().type(),
            paramCtx.methodParam().parameterizedType());

        Object entity = null;

        // First we make sure that the bean-class really is a bean and not some
        // simple type.
        if (!reflectionProvider.isSimpleType(beanClass)) {
            // Attempt to automatically resolve the correct data adapter. For
            // example, if the bean has been annotated
            // with javax.persistence.Entity then we should end up with the
            // JpaDataAdapter.
            DataAdapter dataAdapter = dataAdapterFactory.create(beanClass);

            if (dataAdapter != null) {
                // Now we attempt to automatically retrieve the bean's
                // identifiers. For example in JPA they may be
                ...
                if (beanIdentifiers.isEmpty())
                    throw new IllegalStateException("Unable to auto fetch bean '" + beanClass.getName()
                        + "' from database because no identifier is present.");

                if (beanIdentifiers.size() > 1)
                    throw new IllegalStateException("Unable to auto fetch bean '" + beanClass.getName()
                        + "' from database because only a single identifier is supported.");

                // Currently we only support a single primary key.
                Entry<String, Class<?>> identifier = beanIdentifiers.entrySet().iterator().next();

                // Once we have got the name of the primary key field, we
                // attempt to find a matching parameter in the
                // current request.
                ...
                if ((idValue == null || idValue.isEmpty()) && HttpMethod.PUT.equals(requestCtx.getMethod()))
                    throw new IllegalStateException("You must provide a valid bean identifier for bean '"
                        + beanClass.getName() + "' when using the @Data param annotation and HTTP methodd 'PUT'.");

                // We only deal with primary key types that the SimpleConverter
                // can convert.
                if (idValue != null && !idValue.isEmpty() && simpleConverter.canConvert(identifier.getValue()))
                    entity = dataAdapter.fetch(beanClass,
                        simpleConverter.fromString(idValue.get(0), identifier.getValue()));

                // if the entity is null and we are dealing with a POST request,
                // we attempt to create a new bean
                // instance.
                if (entity == null && HttpMethod.POST.equals(requestCtx.getMethod()))
                    entity = injector.getInstance(beanClass);

                // Now if we have a bean instance and the HTTP-Method is either
                // POST or PUT, we can look for parameters
                // in the current request to populate the bean with.
                if (entity != null && HttpMethod.POST.equals(requestCtx.getMethod())
                    || HttpMethod.PUT.equals(requestCtx.getMethod())) {
                    ...
                    if (beanData != null && !beanData.isEmpty()) {
                        ...
                        if (beanConverter != null) {
                            ConverterContext converterCtx = injector.getInstance(ConverterContext.class).build(
                                paramName, beanClass, null, requestCtx, paramCtx.requestValues(), paramCtx.errors(),
                                paramCtx.notices());

                            beanConverter.bindProperties(beanData, paramName, entity, converterCtx);
                        } else {
                            throw new IllegalStateException("Unable to find a compatible bean converter for the bean '"
                                + beanClass.getName() + "' which is needed for the @ModelParam(" + paramName
                                + ") param when the HTTP method is either 'POST' or 'PUT'.");
                        }
                    }
                }
            }
