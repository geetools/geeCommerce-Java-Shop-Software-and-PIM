...
public class DefaultProductListURIBuilder implements ProductListURIBuilder {
	...
	public FilterValue(Attribute attribute) {
	    ...
	    if (attribute.getProductListFilterKeyAlias() != null && attribute.getProductListFilterKeyAlias().getStr() != null)
	...
	public String toString() {
	    return "FilterValue [attrCode=" + attrCode + ", attrAlias=" + attrAlias + ", values=" + values + ", rangeFrom=" + rangeFrom + ", rangeTo=" + rangeTo + ", position=" + position + ", isMulti=" + isMulti + ", isRange=" + isRange
		    + ", prefix=" + prefix + ", numUnderscores=" + numUnderscores + "]";
	}
    }
	...
	for (String key : keys) {
		...
		ContextObject<String> prefix = ContextObject.valueOf((List<Map<String, Object>>) filterRuleAttr.get(ProductListFilterRule.AttributeField.PREFIX));

		// Override values if they have been specified in the filter-rules collection
		if (posInURI != null)
	...
	for (String key : keys) {
	    ...
	    // If we cannot find the attribute in the filter-rule, it must be a normal parameter filter.
	    if (filterRuleAttr == null) {
	...
	for (FilterValue filterValue : paramFilterValues) {
		...
		if (filterValue.rangeTo != null)
		    filterURL.append(Char.AMPERSAND).append(attrCode).append(TO_SUFFIX).append(Char.EQUALS).append(filterValue.rangeTo);
	    } else {
		if (values.size() > 1) {
