...
public class AddProgrammeProduct implements ImportAction {
    ...
    @Inject
    public AddProgrammeProduct(ImportHelper importHelper, ProductImportHelper productImportHelper, Products products) {
    ...
    public void process(ImportContext importContext) {
        ...
        if (p == null) {
            importContext.add(app.model(ImportMessage.class)
                ...
                .setMessage("addProgrammeProductNotFound")
                .addArg(importContext.field("_id")));

            return;
        }

        Map<String, Object> productIds = productImportHelper.productIds(importContext.data(), "_programme_product");
        ...
        if (productIds == null || productIds.isEmpty()) {
            importContext.add(app.model(ImportMessage.class)
                ...
                .setImportStage(ImportStage.PROCESS)
                .setMessage("addProgrammeProductNotFound")
                .addArg(importContext.field("_programme_product")));
        ...
        Product programmeProduct = products.findById(Product.class, (Id) productIds.get("_id"));

        if (programmeProduct == null) {
            importContext.add(app.model(ImportMessage.class)
                .setToken(importContext.token())
                .setFileName(importContext.fileName())
                .setLineNumber(importContext.lineNumber())
        ...
        p.addProgrammeProduct(programmeProduct);
    }
