...
public class SynonymsGenerator {
    ...
    public List<String> generateInlineSynonymConfiguration(SynonymsHelper.SynonymsConfiguration synonymsConfiguration) {
        List<Map<String, Object>> synonyms = MongoHelper.find(MongoHelper.mongoMerchantDB(), COLLECTION_SYNONYMS,
            Collections.emptyMap(), null);
        List<String> result = new LinkedList<>();
        if (synonymsConfiguration.getOnlyCustom()) {
            synonyms.stream().filter(map -> (Boolean) map.get(FIELD_KEY_CUSTOM))
                .forEach(map -> result.add(createLine(map.get(FIELD_KEY_WORLD).toString(),
                    StringUtils.join((BasicDBList) map.get(FIELD_KEY_SYNONYMS), ","))));
        } else
            synonyms.stream().forEach(map -> result.add(createLine(map.get(FIELD_KEY_WORLD).toString(),
                StringUtils.join((BasicDBList) map.get(FIELD_KEY_SYNONYMS), ","))));
        return result;
    }
}
