...
@Observe(name = "catalog.product.model.Product", event = { Event.AFTER_NEW,
...
public class ProductObserver implements Observer {
    ...
    public void onEvent(Event evt, Observable o) {
        ...
        System.out.println("[" + Thread.currentThread().getName() + "] Running asynchronous observer for: " + productId
            + " -> " + evt.name());
        ...
        if (p.isVariant()) {
            ...
            if (parent != null) {
                System.out.println("[" + Thread.currentThread().getName() + "] Running asynchronous update for parent: "
                    + parent.getId());
