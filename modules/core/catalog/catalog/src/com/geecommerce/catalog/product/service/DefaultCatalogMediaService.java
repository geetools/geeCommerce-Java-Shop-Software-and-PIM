...
public class DefaultCatalogMediaService implements CatalogMediaService {
    ...
    // 
    // @Override
    // public Map<Id, List<ProductImage>> findImagesForProducts(Id... productIds)
    // {
    // ApplicationContext appCtx = app.getApplicationContext();
    //
    // // First attempt to find the most specific product images for this request-context
    // List<ProductImage> foundImages = productImages.thatBelongTo(appCtx.getRequestContext(), productIds);
    //
    // // Filter out product-ids where we already have the images
    // Id[] nextIds = productHelper.filterCompletedProductIds(foundImages, productIds);
    //
    // // For store
    // if (nextIds != null && nextIds.length > 0)
    // {
    // // Then attempt to find images on store-level
    // List<ProductImage> foundStoreImages = productImages.thatBelongTo(appCtx.getStore(), nextIds);
    //
    // foundImages.addAll(foundStoreImages);
    //
    // // Filter out product-ids where we already have the images
    // nextIds = productHelper.filterCompletedProductIds(foundStoreImages, nextIds);
    // }
    //
    // // For merchant
    // if (nextIds != null && nextIds.length > 0)
    ...
    // List<ProductImage> foundMerchantImages = productImages.thatBelongTo(appCtx.getMerchant(), nextIds);
    //
    // foundImages.addAll(foundMerchantImages);
    //
    // // Filter out product-ids where we already have the images
    // nextIds = productHelper.filterCompletedProductIds(foundMerchantImages, nextIds);
    // }
    //
    // // Now try the most global way which is for all, regardles of merchant, store or request-context
    // if (nextIds != null && nextIds.length > 0)
    // {
    // // Then attempt to find images on global-level
    // List<ProductImage> foundGlobalImages = productImages.thatBelongTo(nextIds);
    //
	...
	if (catalogMediaAsset != null && catalogMediaAsset.getId() != null && catalogMediaAsset.getProductId() != null) {
	    // If media asset has been disabled, we also set the other flags to false.
	    ...
	    List<CatalogMediaAsset> productImages = catalogMedia.allImagesFor(catalogMediaAsset.getProduct(), mediaTypeMap.get(MediaType.PRODUCT_MAIN_IMAGE), mediaTypeMap.get(MediaType.PRODUCT_LIST_IMAGE1),
