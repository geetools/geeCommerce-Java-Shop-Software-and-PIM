...
public class ProductDecorWidget extends AbstractWidgetController implements WidgetController {
    ...
    public void execute(WidgetContext widgetCtx, HttpServletRequest request, HttpServletResponse response,
        ...
        if (productId != null) {
            ...
            if (product.isVariantMaster()) {
                ...
                List<String> decorAttributes = Arrays.asList(new String[] { "dekor_kombination", "jalousien_farbe",
                    "rollo_farbe", "teppichfarbe", "wohndecken_farbe", "suchfarbe" });
                ...
                List<Product> validVariants = variants.stream().filter(v -> v.isValidForSelling())
    ...
    private List<AttributeValue> getVariantAttributes(Product product) {
         ...
         * product.getVariantAttributes().size() > 0) return
