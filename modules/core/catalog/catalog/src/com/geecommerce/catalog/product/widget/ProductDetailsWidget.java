...
public class ProductDetailsWidget extends AbstractWidgetController implements WidgetController {
	...
	if (productId != null) {
		...
		List<AttributeValue> attributeValues = attributeGroup.getItems().stream().filter(item -> item.getType().equals(AttributeGroupMappingType.ATTRIBUTE)).map(item -> p.getAttribute(item.getId(), true)).filter(x -> x != null).collect(Collectors.toList());
