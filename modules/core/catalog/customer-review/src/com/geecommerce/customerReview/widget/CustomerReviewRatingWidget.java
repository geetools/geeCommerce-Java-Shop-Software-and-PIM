...
public class CustomerReviewRatingWidget extends AbstractWidgetController implements WidgetController {
    ...
    public void execute(WidgetContext widgetCtx, HttpServletRequest request, HttpServletResponse response,
        ...
        widgetCtx.setParam("average",
            String.format(Locale.ENGLISH, "%.2f", customerReviewService.averageRating(productId)));
 ...
 * 
 * @Inject public AddToWishListWidget(WishListService wishListService){
 * this.wishListService = wishListService; }
 * 
 * @Override public void execute(WidgetContext widgetCtx, HttpServletRequest
 * request, HttpServletResponse response, ServletContext servletContext) throws
 * Exception { String productIdParam = widgetCtx.getParam(PARAM_PRODUCT_ID);
 * if(productIdParam !=null && !productIdParam.isEmpty())
 * widgetCtx.setParam("productId", productIdParam);
 * 
 * if(app.isCustomerLoggedIn()){ widgetCtx.setParam("wishLists",
