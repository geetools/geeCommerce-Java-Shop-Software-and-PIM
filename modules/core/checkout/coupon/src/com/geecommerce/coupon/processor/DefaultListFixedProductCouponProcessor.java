...
public class DefaultListFixedProductCouponProcessor extends BaseListProductCouponProcessor implements CouponProcessor {
	...
	if (nullCheck(couponCode) && couponCode.getCoupon().getCouponAction().getType().equals(CouponActionType.LIST_FIXED_PRODUCT))
	    return true;
	return false;
    }
	...
	if ((Integer) itemDiscount.get(CalculationItemDiscount.FIELD.DISCOUNT_ITEM_QUANTITY) == 0)
	    itemDiscount.put(CalculationItemDiscount.FIELD.DISCOUNT_ITEM_AMOUNT, couponAction.getDiscountAmounts().get(itemIndex));
	else {
