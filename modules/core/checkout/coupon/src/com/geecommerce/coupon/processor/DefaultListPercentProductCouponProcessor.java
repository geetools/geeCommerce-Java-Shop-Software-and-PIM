...
public class DefaultListPercentProductCouponProcessor extends BaseListProductCouponProcessor
    implements CouponProcessor {
    @Override
    public boolean canBeProcessed(CalculationContext calcCtx, CouponCode couponCode,
        ...
        if (nullCheck(couponCode)
            && couponCode.getCoupon().getCouponAction().getType().equals(CouponActionType.LIST_PERCENT_PRODUCT))
            return true;
        return false;
    }
    ...
    protected void setDiscount(Map<String, Object> itemDiscount, CouponAction couponAction, int itemIndex) {
        if ((Integer) itemDiscount.get(CalculationItemDiscount.FIELD.DISCOUNT_ITEM_QUANTITY) == 0)
            itemDiscount.put(CalculationItemDiscount.FIELD.DISCOUNT_ITEM_RATE,
