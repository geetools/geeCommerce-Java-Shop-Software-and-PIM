    ...
    public void execute(WidgetContext widgetCtx, HttpServletRequest request, HttpServletResponse response,
        ...
        if (webSlideShowName != null && !webSlideShowName.isEmpty()) {
            ...
            if (slideShows != null && !slideShows.isEmpty()) {
                Optional<WebSlideShow> optional = slideShows.stream()
                    .filter(x -> (x.getDateFrom() == null || x.getDateFrom().before(new Date()))
                        && (x.getDateTo() == null || x.getDateTo().after(new Date())))
                    .findFirst();
                ...
                if (slideShow != null) {
                    ...
                    if (slides != null && !slides.isEmpty()) {
                        slides = slides.stream()
                            .filter(slide -> (slide.getShowFrom() == null || slide.getShowFrom().before(new Date()))
                                && (slide.getShowTo() == null || slide.getShowTo().after(new Date())))
        ...
        /*
         * if (promotionTemplate.equals("web_slideshow")) { int
         * slideshowInterval = app.cpInt_("home/web_slideshow/interval", 8000);
         * 
         * if (slideshowInterval < 1000) { slideshowInterval = 8000; }
         * 
