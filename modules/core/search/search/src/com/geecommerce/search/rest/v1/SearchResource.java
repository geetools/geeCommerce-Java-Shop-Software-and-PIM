...
public class SearchResource extends AbstractResource {
    ...
    @Inject
    public SearchResource(RestService service, SearchService searchService, SearchHelper searchHelper, ProductService productService, ElasticsearchHelper elasticsearchHelper) {
	...
	SearchResult searchResult = searchService.query(new SearchQuery(query, null, filter.getOffset() == null ? 0 : filter.getOffset().intValue(), filter.getLimit() == null ? 25 : filter.getLimit()));
	...
	if (searchResult != null && searchResult.getDocumentIds() != null && searchResult.getDocumentIds().size() > 0) {
	    ...
	    products = productService.getProducts(Arrays.asList(elasticsearchHelper.toIds(searchResult.getDocumentIds().toArray())), queryOptions);
	}
