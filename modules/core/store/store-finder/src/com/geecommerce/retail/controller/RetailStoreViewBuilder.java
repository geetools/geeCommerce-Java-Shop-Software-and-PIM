...
public class RetailStoreViewBuilder {
    ...
    public List<RetailStoreView> createRetailStoreViewWithDistances(List<RetailStore> retailStores, String customerZipCode) {
	List<RetailStoreView> retailStoreViews = new LinkedList();
	retailStores.forEach(retailStore -> retailStoreViews.add(createRetailStoreView(retailStore).calculateDistance(new LocationService.Location(retailStore.attr("latitude").getDouble(), retailStore.attr("longitude").getDouble()), customerZipCode)
		.build()));
	return retailStoreViews;
    }
